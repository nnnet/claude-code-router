...
- заметки: добавлена функция `zed_launch()`: если ключа нет в окружении, но он
  есть у systemd-менеджера, Zed порождается транзиентной user-службой
  (`systemd-run --user --collect --same-dir --property=KillMode=process`) —
  переменные подставляет сам systemd, скрипт значение не читает. `KillMode=process`
  обязателен: CLI-обёртка zed завершается после порождения zed-editor.
  Выключатель — `ZED_NO_SYSTEMD_RUN=1`. Проверено запуском из оболочки без ключа
  (эквивалент запуска из меню): у `zed-editor` ровно один `*_API_KEY` —
  `LOCAL_ROUTER_API_KEY`, cgroup `app.slice/run-r*.service`. Пользователь
  проверил оба пути — из меню GNOME и `~/.config/zed/zed.sh`: модели видны и
  отвечают. Долговременный источник — `~/.config/environment.d/zed-local-router.conf`,
  генератор systemd его отдаёт.
  Постфикс: диагностика врала — печатала «ключ не задан, модели не появятся»
  прямо перед строкой, где systemd его подставляет. Добавлен предикат
  `zed_key_in_systemd_env()` (проверяет только наличие имени), его же использует
  `zed_launch`; блок подсказок теперь показывается только когда ключа нет нигде
